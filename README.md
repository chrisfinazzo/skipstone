...
The same binary is used by the SwiftPM and Xcode Skip build plugin
as well as the command-line tool installed via Homebrew's `brew install skiptools/skip/skip`

> [!CAUTION]
> This repository, https://github.com/skiptools/skipstone.git, creates the `skip` tool,
> whereas the https://github.com/skiptools/skip.git repository hosts the `skipstone` plugin.
> The names are the reverse of what you might expect.

The `skip` tool itself contains a plethora of commands that support the
